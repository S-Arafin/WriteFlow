...
export function EditorClient({ document, userPlan }: EditorClientProps) {
  ...
  const handleTitleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
    if (editor) triggerAutoSave(val, editor.getHTML());
  };

  const startDraftingStream = async () => {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
    if (!draftPrompt.trim() || !editor) return;
  ...
  const executeSelectionRewrite = async (tone: string) => {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
    if (!selectedText.trim() || isRewriting || !editor) return;
  ...
  const sendChatMessage = async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
    const documentContext = editor ? editor.getText().slice(0, 3000) : '';
